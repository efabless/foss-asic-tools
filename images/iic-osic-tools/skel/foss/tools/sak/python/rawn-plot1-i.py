...
try:
    root = tkinter.Tk()
    root.wm_title("rawn-plot1")
    can_gui = True
    print("can_gui is True\n")
except:
    can_gui = False
    print("can_gui is False\n")
...
def usage(msg = None):
    ...
    [-i | -b]        : set interactive or batch mode. Automatic if invoked as rawploti or {rawplotb, rawn-plot1}.
...
def sizeForLegendFail(doLegend):
    global fig
    [w, h] = fig.get_size_inches()
    fig.set_size_inches(w, h, forward=True)
    
def sizeForLegend(doLegend):
    global fig
    if doLegend:
...
def toggle_legend():
    global fLegend, doLegend
    doLegend = not doLegend
    sizeForLegend(doLegend)
    fLegend.set_visible(doLegend)
    canvas_redraw()

def main(argv):

    global doLegend, fLegend, canvas, toolbar, fig, can_gui
    ...
    if progName in ['rawn-plot1', 'rawplotb']:
        do_gui = False
    elif progName in ['rawploti']:
        do_gui = True
    else:
        do_gui = can_gui
    ...
    try:
        opts, args = getopt.getopt(argv[1:], "GPhviblLD:g:A:s:o:T:X:Y:S:W:H:", ['xscale=', 'yscale='])
    ...
    if not can_gui and do_gui:
        err += 1
        perr( "ERROR: GUI-mode requested but is not available; use -h for usage" )
        sys.exit(err)
    ...
    fLegend = ax.legend(handles, labels, loc='upper center', ncol=3, columnspacing=0.1, bbox_to_anchor=(0.5,-0.15))
    ...
    if can_gui:
        ...
        brow = tkinter.Frame(root)
        brow.pack(side=tkinter.BOTTOM, fill=tkinter.X)
        buttonq = tkinter.Button(master=brow, text="X", command=_quit)
        buttonq.pack(side=tkinter.LEFT, expand=0)
        buttonl = tkinter.Button(master=brow, text="legend", command=toggle_legend)
        buttonl.pack(side=tkinter.LEFT, expand=0)

        toolbar = NavigationToolbar2Tk(canvas, brow)
        toolbar.pack(side=tkinter.RIGHT, fill=tkinter.X, expand=1)
        toolbar.update()
